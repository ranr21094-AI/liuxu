...
- `lib/workspace/zip.js` centralizes ZIP limits: compressed 512 MiB, SQLite 120 MiB, each binary 30 MiB, legacy JSON 10 MiB, total declared expansion 1 GiB, 20,000 entries. Export checks the same limits. Native ZIP imports use validated SQLite, ignoring redundant JSON; legacy imports stage all JSON in isolation. Restore snapshots, rolls back on installation failure, removes the derived search index, and invalidates service/runtime caches.
