...
async function exportWorkspace(db) {
  ...
  addDirToZip(zip, path.join(db.dataDir, 'ai-media'), 'ai-media');
  ...
  const nextNoteId = documents.reduce((max, item) => {
    const match = /^note:(\d+)$/.exec(item.id || '');
    return match ? Math.max(max, Number(match[1]) + 1) : max;
  }, 1);
  const nextFileId = documents.reduce((max, item) => {
...
async function restoreWorkspace(db, buffer, mode = 'replace') {
  ...
  await extractDir('ai-media/', path.join(db.dataDir, 'ai-media'));
