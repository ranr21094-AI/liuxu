...
function filterDocuments(documents, filters = {}) {
  ...
  if (filters.folder !== undefined) {
    // Exact folder match ("folder" param): empty string = base root only
    const target = String(filters.folder);
    list = list.filter(doc => String(doc.folderPath || '') === target);
  }
...
function filtersFromQuery(query = {}) {
  return {
    ...
    folder: typeof query.folder === 'string' ? String(query.folder).trim() : undefined,
