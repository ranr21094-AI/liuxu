...
const { extractText, extractDocxPreview, inferPreviewKind } = require('./import');
...
function createKnowledgeService(db) {
  ...
  async function importBinaryFromDisk(input) {
    const buffer = fs.readFileSync(input.fullPath);
    const extracted = await extractText(buffer, input.filename, '');
    ...
    const document = normalizeDocument({
      ...
      status: input.status || extracted.status || 'active',
      previewHtml: extracted.previewHtml || '',
      fileMeta: { ...existing.fileMeta, filename: input.filename, mimeType: extracted.mimeType || 'application/octet-stream', bytes: buffer.length, sha256: digest, storedName, previewKind: extracted.previewKind || inferPreviewKind(extracted.mimeType, input.filename) },
      updatedAt: nowIso(),
      version: (existing.version || 1) + 1,
      fileSync: { relativePath: input.relativePath, fingerprint: input.fingerprint, sourceType: 'file', parentDocumentId: null },
    });
