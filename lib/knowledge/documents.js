...
const { extractText, extractDocxPreview, inferPreviewKind, MAX_PARSE_BYTES } = require('./import');
const { fileKind } = require('./file-formats');
const { KnowledgeFolderSync } = require('./folder-sync');
const {
  canonicalizeContent,
  normalizeTitle,
  parseWikiLinks,
...
function markDiaryUploads(db, content, visibility) {
  if (visibility === 'diary' && typeof db.markPrivateUploadsFromContent === 'function') {
    db.markPrivateUploadsFromContent(content);
  }
}
...
function createKnowledgeService(db) {
  ...
  function restoreRevision(id, revisionId, { baseVersion, diaryUnlocked = false } = {}) {
    ...
    const visibility = isDiaryCollection(collectionPath, db) ? 'diary' : 'standard';
    ...
    markDiaryUploads(db, updated.content, updated.visibility);
  ...
  function writeDocument(doc, {
    previous = null,
    revisionReason = 'auto',
    skipFolderSync = false,
    syncRollbackStack = null,
  } = {}) {
    const ownsRollbackStack = !syncRollbackStack;
    const rollbackStack = syncRollbackStack || [];
    if (folderSync && !skipFolderSync) {
      const prepared = folderSync.prepareWrite(doc, previous);
      Object.assign(doc, prepared);
      rollbackStack.push({ previous, next: prepared });
    }
    const tx = sqlite.transaction(() => {
      ...
      if (doc.sourceType === 'file') {
        for (const child of nativeDocuments().filter(item => item.parentDocumentId === doc.id)) {
          const visibility = doc.visibility === 'diary' ? 'diary' : child.visibility;
          if (child.collectionPath === doc.collectionPath && child.visibility === visibility) continue;
          const next = { ...child, knowledgeBase: doc.knowledgeBase, folderPath: doc.folderPath,
            collectionPath: doc.collectionPath, visibility, updatedAt: nowIso(), version: (child.version || 1) + 1 };
          writeDocument(next, { previous: child, syncRollbackStack: rollbackStack });
          markDiaryUploads(db, next.content, next.visibility);
        }
      }
    });
    try {
      ...
      if (ownsRollbackStack && folderSync && !skipFolderSync) {
        for (const item of rollbackStack.reverse()) {
          try {
            if (item.previous) folderSync.prepareWrite(item.previous, item.next);
            else folderSync.removeDocument(item.next);
          } catch { /* journal forces a later full reconciliation */ }
        }
      }
      throw error;
    }
  ...
  function writeStore(store, { skipFolderSync = false } = {}) {
    const parentAdjusted = new Map();
    const parents = new Map(store.documents.filter(doc => doc.sourceType === 'file').map(doc => [doc.id, doc]));
    store.documents = store.documents.map(child => {
      const parent = parents.get(child.parentDocumentId);
      if (!parent) return child;
      const visibility = parent.visibility === 'diary' ? 'diary' : child.visibility;
      if (child.collectionPath === parent.collectionPath && child.visibility === visibility) return child;
      const next = { ...child, collectionPath: parent.collectionPath, knowledgeBase: parent.knowledgeBase,
        folderPath: parent.folderPath, visibility, version: (child.version || 1) + 1, updatedAt: nowIso() };
      parentAdjusted.set(child.id, child);
      markDiaryUploads(db, next.content, next.visibility);
      return next;
    });
    const preparedPairs = [];
    const rollbackPrepared = () => {
      for (const item of [...preparedPairs].reverse()) {
        try { folderSync.prepareWrite(item.previous, item.next); } catch { /* journal forces a later full reconciliation */ }
      }
    };
    if (folderSync && !skipFolderSync) {
      const existingDocuments = new Map(nativeDocuments().map(document => [document.id, document]));
      try {
        store.documents = store.documents.map(document => {
          const previous = existingDocuments.get(document.id);
          if (!previous || JSON.stringify(previous) === JSON.stringify(document)) return document;
          const next = folderSync.prepareWrite(document, previous);
          preparedPairs.push({ previous, next });
          return next;
        });
      } catch (error) {
        rollbackPrepared();
    ...
    const tx = sqlite.transaction(() => {
      parentAdjusted.forEach(previous => captureRevision(previous, 'auto'));
    ...
    try { tx(); } catch (error) { rollbackPrepared(); throw error; }
  }

  function documentTimestamp(doc) {
    const updated = Date.parse(String(doc?.updatedAt || ''));
    if (Number.isFinite(updated)) return updated;
    const created = Date.parse(String(doc?.createdAt || ''));
    return Number.isFinite(created) ? created : 0;
  }

  function mergeNativeDocuments(documents = [], ids = {}) {
    for (const raw of documents) {
      if (!raw || typeof raw.id !== 'string' || !['note', 'file'].includes(raw.sourceType)) continue;
      const doc = normalizeDocument(raw);
      const existing = readDocumentById(doc.id);
      if (existing && documentTimestamp(doc) < documentTimestamp(existing)) continue;
      writeDocument(doc, { previous: existing, revisionReason: 'restore_merge' });
    }
    const current = readStore();
    const nextNoteId = Math.max(current.nextNoteId, Math.max(1, Number(ids.nextNoteId) || 1));
    const nextFileId = Math.max(current.nextFileId, Math.max(1, Number(ids.nextFileId) || 1));
    writeMeta(sqlite, 'next_note_id', String(nextNoteId));
    writeMeta(sqlite, 'next_file_id', String(nextFileId));
  }
  ...
  function allDocuments({ diaryUnlocked = false, includeArchived = false } = {}) {
    ...
    return docs.filter(doc => {
      ...
      if (doc.parentDocumentId) {
        const parent = byId.get(doc.parentDocumentId);
        if (!parent || (parent.visibility === 'diary' && !diaryUnlocked)) return false;
      }
      return true;
    });
  }
  ...
  function getDocument(id, { diaryUnlocked = false } = {}) {
    ...
    if (doc.parentDocumentId) {
      const parent = readDocumentById(doc.parentDocumentId);
      if (!parent || (parent.visibility === 'diary' && !diaryUnlocked)) return null;
    }
    return { ...doc };
  }
  ...
  function createNote(input, { diaryUnlocked = false } = {}) {
    ...
    const doc = sqlite.transaction(() => {
      ...
      writeDocument(created);
      writeMeta(sqlite, 'next_note_id', String(nextNoteId + 1));
      return created;
    })();
    markDiaryUploads(db, doc.content, doc.visibility);
  ...
  function updateDocument(id, patch, { diaryUnlocked = false } = {}) {
    ...
    try {
      ...
      if (error?.code === 'FILE_SYNC_CONFLICT') {
        return { error: error.message, status: 409, current: readDocumentById(id) || current, draftSaved: true };
      }
      throw error;
    }
    markDiaryUploads(db, updated.content, updated.visibility);
  ...
  function deleteDocument(id, { diaryUnlocked = false } = {}) {
    ...
    if (current.sourceType === 'file') {
      ...
      const ids = store.documents
        ...
        .map(item => item.id);
      try { ids.forEach(itemId => folderSync?.removeDocument(store.documents.find(item => item.id === itemId))); }
      catch (error) { return { error: error.message, status: 500 }; }
      const remove = sqlite.prepare('DELETE FROM knowledge_documents WHERE id = ?');
      const tx = sqlite.transaction(() => {
  ...
  function rewriteCollectionPath(oldPath, newPath) {
    ...
    if (changed) {
      writeStore(store);
      store.documents.forEach(doc => markDiaryUploads(db, doc.content, doc.visibility));
    }
    return changed;
  }
  ...
  function saveImportedFile({
    ...
    const bytes = Number.isFinite(Number(sourceBytes)) ? Number(sourceBytes) : (buffer?.length || 0);
    if (!buffer && !sourceFilePath) return { error: 'File data is missing', status: 400 };
    const sha256 = sourceSha256 || crypto.createHash('sha256').update(buffer).digest('hex');
    ...
    if (sourceFilePath) fs.copyFileSync(sourceFilePath, storedPath);
    else fs.writeFileSync(storedPath, buffer);
    const now = nowIso();
    ...
    const doc = sqlite.transaction(() => {
      ...
      const created = {
        ...
        title: title || filename || `file:${nextFileId}`,
        content: normalizedContent.content,
        ...
        fileMeta: {
          ...
          previewLimited: Boolean(previewLimited),
        },
        createdAt: now,
        updatedAt: now,
        version: 1,
  ...
  function filePathFor(doc) {
    const synchronized = folderSync?.localPathFor(doc);
    if (synchronized && fs.existsSync(synchronized)) return synchronized;
  ...
  function deleteFromDisk(id) {
    const current = readDocumentById(id);
    ...
    const tx = sqlite.transaction(() => {
      for (const itemId of ids) {
        sqlite.prepare('DELETE FROM knowledge_documents WHERE id = ?').run(itemId);
        sqlite.prepare('DELETE FROM knowledge_link_targets WHERE document_id = ?').run(itemId);
      }
      touchIndexVersion(ids.map(documentId => ({ documentId, operation: 'delete' })));
    });
    tx();
  ...
  function importMarkdownFromDisk(input) {
    const currentNext = Math.max(1, Number(readMeta(sqlite, 'next_note_id', '1')) || 1);
    const requested = /^note:[1-9]\d*$/.test(String(input.id || '')) && !readDocumentById(input.id) ? input.id : '';
    const id = requested || `note:${currentNext}`;
    const numericId = Number(String(id).split(':')[1]) || currentNext;
    ...
    const document = normalizeDocument({
      ...
      sourceType: 'note',
      sourceRef: id,
      title: String(input.title || '未命名笔记').slice(0, 200),
      content: String(input.content || ''),
      tags: Array.isArray(input.tags) ? input.tags.slice(0, 20) : [],
      status: input.status === 'archived' ? 'archived' : 'active',
      visibility: input.visibility === 'diary' ? 'diary' : 'standard',
      createdAt: timestamp,
      updatedAt: timestamp,
      version: 1,
      parentDocumentId: input.parentDocumentId || null,
      documentRole: input.documentRole === 'annotation' ? 'annotation' : 'normal',
    });
    writeDocument(document, { skipFolderSync: true });
    writeMeta(sqlite, 'next_note_id', String(Math.max(currentNext, numericId + 1)));
    markDiaryUploads(db, document.content, document.visibility);
    return document;
  }

  function upsertFromDisk(id, input) {
    const previous = readDocumentById(id);
    if (!previous) return importMarkdownFromDisk({ ...input, id });
    const document = normalizeDocument({
      ...previous,
      ...input,
      id: previous.id,
    ...
    writeDocument(document, { previous, revisionReason: 'external_sync', skipFolderSync: true });
    markDiaryUploads(db, document.content, document.visibility);
    return document;
  }

  async function importBinaryFromDisk(input) {
    const size = Number(input.fingerprint?.size) || fs.statSync(input.fullPath).size;
    const buffer = size <= MAX_PARSE_BYTES ? fs.readFileSync(input.fullPath) : null;
    const kind = fileKind(input.filename, '');
    const extracted = buffer
      ? await extractText(buffer, input.filename, '')
      : { text: '', status: 'active', previewKind: kind.previewKind, mimeType: kind.mimeType, previewLimited: true };
    if (extracted.error) throw new Error(extracted.error);
    const existing = input.existing;
    if (!existing) {
      const currentNext = Math.max(1, Number(readMeta(sqlite, 'next_file_id', '1')) || 1);
      const id = `file:${currentNext}`;
      const digest = input.fingerprint?.sha256 || crypto.createHash('sha256').update(buffer).digest('hex');
      const storedName = `${digest.slice(0, 16)}-${safeStoredFilename(input.filename)}`;
      fs.mkdirSync(filesDir(), { recursive: true });
      fs.copyFileSync(input.fullPath, path.join(filesDir(), storedName));
      const timestamp = nowIso();
      const document = normalizeDocument({
        id,
        sourceType: 'file',
        ...
        tags: [],
        visibility: input.knowledgeBase === '日记' ? 'diary' : 'standard',
        status: input.status || extracted.status || 'active',
        ...
        fileMeta: {
          ...
          sha256: digest,
          storedName,
          previewKind: extracted.previewKind || inferPreviewKind(extracted.mimeType, input.filename),
        ...
        version: 1,
        documentDate: '',
        fileSync: { relativePath: input.relativePath, fingerprint: input.fingerprint, sourceType: 'file', parentDocumentId: null },
      });
      writeDocument(document, { skipFolderSync: true });
      writeMeta(sqlite, 'next_file_id', String(currentNext + 1));
      return document;
    }
    const digest = input.fingerprint?.sha256 || crypto.createHash('sha256').update(buffer).digest('hex');
    const storedName = `${digest.slice(0, 16)}-${safeStoredFilename(input.filename)}`;
    ...
    const document = normalizeDocument({
      ...
      folderPath: input.folderPath,
      collectionPath: input.collectionPath,
      visibility: input.knowledgeBase === '日记' ? 'diary' : existing.visibility,
      status: input.status || extracted.status || 'active',
      previewHtml: extracted.previewHtml || '',
      fileMeta: { ...existing.fileMeta, filename: input.filename, mimeType: extracted.mimeType || 'application/octet-stream', bytes: size, sha256: digest, storedName, previewKind: extracted.previewKind || inferPreviewKind(extracted.mimeType, input.filename), previewLimited: Boolean(extracted.previewLimited) },
      updatedAt: nowIso(),
      version: (existing.version || 1) + 1,
      fileSync: { relativePath: input.relativePath, fingerprint: input.fingerprint, sourceType: 'file', parentDocumentId: null },
    });
    writeDocument(document, { previous: existing, revisionReason: 'external_sync', skipFolderSync: true });
    return document;
  }

  function ensureCategoryPath(value) {
    const segments = String(value || '').split('/').filter(Boolean);
    if (!segments.length) return;
    let prefix = '';
    for (const segment of segments) {
      const parent = prefix;
      const current = prefix ? `${prefix}/${segment}` : segment;
      const all = db.getAllCategories(true, true);
      const exists = (() => {
        let nodes = all;
      ...
      if (!exists) db.addCategory(segment, parent || undefined);
      prefix = current;
    }
  }

  function removeMissingCategories(present) {
    const flatten = (nodes, prefix = '', output = []) => {
      for (const node of nodes) {
        const value = prefix ? `${prefix}/${node.name}` : node.name;
        output.push(value);
        flatten(node.sub || [], value, output);
      }
      return output;
    };
    const paths = flatten(db.getAllCategories(true, true)).sort((a, b) => b.split('/').length - a.split('/').length);
    for (const value of paths) {
      if (value === '其他' || value === '日记' || present.has(value)) continue;
      db.deleteCategory(value);
    }
  }

  folderSync = new KnowledgeFolderSync({
    db,
    adapter: {
      listDocuments: nativeDocuments,
      readDocument: readDocumentById,
      sourceFilePath(document) {
        if (!document?.fileMeta?.storedName) return null;
        const target = path.resolve(filesDir(), document.fileMeta.storedName);
        return target.startsWith(`${path.resolve(filesDir())}${path.sep}`) ? target : null;
      },
      importMarkdown: importMarkdownFromDisk,
      upsertFromDisk,
      importBinary: importBinaryFromDisk,
      deleteFromDisk,
      ensureCategory: ensureCategoryPath,
      removeMissingCategories,
      recordPreparedDocument(prepared, previous) {
        writeDocument(prepared, { previous, revisionReason: 'folder_migration', skipFolderSync: true });
      },
    },
  });
