...
function documentMatchesFolder(documentFolderPath, folderPath) {
  const docPath = String(documentFolderPath || '');
  if (!folderPath) return !docPath;
...
function treeForDocuments(categories, documents, db) {
  ...
  documents.forEach(document => {
    ...
    if (!base._folders.has(folderPath)) {
      const item = { name: folderPath, path: folderPath, documentCount: 0 };
      base.folders.push(item);
      base._folders.set(folderPath, item);
    }
  });
  for (const base of byBase.values()) {
    const baseName = base.name;
    for (const folder of base.folders) {
      folder.documentCount = documents.filter(document => (
        (document.knowledgeBase || '其他') === baseName
        && documentMatchesFolder(document.folderPath, folder.path)
      )).length;
    }
  }
