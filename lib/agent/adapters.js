...
// The bound document is the working object of a note_assist run, so it gets a
// much larger read budget than @ mention injection (8000 chars).
const NOTE_CONTENT_CHAR_LIMIT = 100000;
const NOTE_EDIT_CHAR_LIMIT = 20000;
...
function createToolAdapters({ db, knowledgeSearch, hasDiaryAccessFlag, webSearch, webFetch, westockRun, imageGenerate, computer, chrome, noteBrowser, memory, agentSettingsFor }) {
  // Per-call diary permission context (snapshotted when the run started).
  // AsyncLocalStorage keeps concurrent tool executions isolated.
  const diaryContextStorage = new AsyncLocalStorage();
  ...
  function diaryUnlocked() {
    const context = diaryContextStorage.getStore();
    if (context && typeof context.diaryUnlocked === 'boolean') return context.diaryUnlocked;
  ...
  async function knowledgeBacklinksTool(args = {}) {
    const { knowledge } = knowledgeSearch || serviceFor(db);
    const id = String(args.id || '').trim();
    if (!id) return toolResult({ ok: false, summary: 'id is required', errorCode: 'invalid' });
    const result = knowledge.backlinks(id, {
      diaryUnlocked: diaryUnlocked(),
      cursor: Math.max(0, Number(args.offset) || 0),
      limit: Math.min(50, Math.max(1, Number(args.limit) || 20)),
    });
    if (result.error) return toolResult({ ok: false, summary: result.error, errorCode: result.status === 403 ? 'denied' : 'not_found' });
    return toolResult({
      ok: true,
      summary: result.backlinks.length ? `Found ${result.backlinks.length} backlink(s)` : 'No backlinks found',
      data: result,
      evidence: result.backlinks.slice(0, 8).map(item => ({ type: 'document', id: item.sourceId })),
    });
  }
  ...
  async function memoryListTool(args = {}) {
    ...
    const offset = Math.max(0, Number(args.offset) || 0);
    const all = memory.list(layer ? { layer } : {}).filter(item => item.kind !== 'system' && item.layer !== 'system');
  ...
  async function memoryReadTool(args = {}) {
    ...
    const id = String(args.id || '').trim();
    if (!id) return toolResult({ ok: false, summary: 'id is required', errorCode: 'invalid' });
    const item = memory.list().find(entry => entry.id === id && entry.kind !== 'system' && entry.layer !== 'system');
  ...
  async function memorySearchTool(args = {}) {
    ...
    const items = memory.list(layer ? { layer } : {})
      .filter(item => item.kind !== 'system' && item.layer !== 'system')
  ...
  // Current-document context for note_assist runs: `{ documentId, onProposal }`.
  function noteContextFrom(context) {
    const note = context && typeof context.note === 'object' ? context.note : null;
    return note && note.documentId ? note : null;
  }

  async function noteReadTool(args, noteContext) {
    const { knowledge } = knowledgeSearch || serviceFor(db);
    const doc = knowledge.getDocument(noteContext.documentId, { diaryUnlocked: diaryUnlocked() });
    if (!doc) return toolResult({ ok: false, summary: 'Current document not found or locked', errorCode: 'not_found' });
    const content = String(doc.content || '');
    const limit = NOTE_CONTENT_CHAR_LIMIT;
    const truncated = content.length > limit;
    return toolResult({
      ok: true,
      summary: `Read current document "${doc.title}"`,
      data: {
        id: doc.id,
        title: doc.title,
        ...
        content: truncated ? `${content.slice(0, limit)}\n…[content truncated at ${limit} characters]` : content,
      },
      evidence: [{ type: 'document', id: doc.id }],
    });
  }

  async function noteProposeEditTool(args, noteContext) {
    const { knowledge } = knowledgeSearch || serviceFor(db);
    const doc = knowledge.getDocument(noteContext.documentId, { diaryUnlocked: diaryUnlocked() });
    if (!doc) return toolResult({ ok: false, summary: 'Current document not found or locked', errorCode: 'not_found' });
    if (doc.status === 'archived') {
      return toolResult({ ok: false, summary: 'Current document is archived', errorCode: 'archived' });
    }
    const content = String(doc.content || '');
    const proposal = { id: `prop_${crypto.randomUUID().replace(/-/g, '').slice(0, 12)}`, documentId: doc.id };
    if (args.append === true) {
      const addition = String(args.content || '');
      if (!addition.trim()) {
        return toolResult({ ok: false, summary: 'content is required when append is true', errorCode: 'invalid' });
      }
      if (addition.length > NOTE_EDIT_CHAR_LIMIT) {
        return toolResult({ ok: false, summary: `content exceeds ${NOTE_EDIT_CHAR_LIMIT} characters`, errorCode: 'too_large' });
      }
      proposal.append = true;
      proposal.content = addition;
      proposal.proposedContent = content ? `${content.replace(/\s+$/, '')}\n\n${addition}` : addition;
      proposal.summary = `Append ${addition.length} characters to the end`;
    } else {
      const find = String(args.find ?? '');
      const replace = String(args.replace ?? '');
      if (!find) {
        return toolResult({ ok: false, summary: 'find is required (or set append true)', errorCode: 'invalid' });
      }
      if (find.length > NOTE_EDIT_CHAR_LIMIT || replace.length > NOTE_EDIT_CHAR_LIMIT) {
        return toolResult({ ok: false, summary: `find/replace exceed ${NOTE_EDIT_CHAR_LIMIT} characters`, errorCode: 'too_large' });
      }
      let occurrences = 0;
      let index = content.indexOf(find);
      while (index !== -1) {
        occurrences += 1;
        index = content.indexOf(find, index + Math.max(1, find.length));
      }
      if (occurrences === 0) {
        return toolResult({ ok: false, summary: 'find does not match the current document content; read the document again and copy the text exactly', errorCode: 'find_not_found' });
      }
      if (occurrences > 1) {
        return toolResult({ ok: false, summary: `find matches ${occurrences} locations; include more surrounding text so it matches exactly once`, errorCode: 'find_ambiguous' });
      }
      proposal.find = find;
      proposal.replace = replace;
      proposal.proposedContent = content.replace(find, () => replace);
      proposal.summary = `Replace ${find.length} characters with ${replace.length} characters`;
    }
    proposal.baseContentLength = content.length;
    proposal.proposedContentLength = proposal.proposedContent.length;
    if (typeof noteContext.onProposal === 'function') noteContext.onProposal(proposal);
    return toolResult({
      ok: true,
      summary: `Edit proposal delivered to the user for preview — ${proposal.summary}. The user applies it manually.`,
      data: { proposalId: proposal.id, summary: proposal.summary },
      evidence: [{ type: 'document', id: doc.id }],
    });
  }
  ...
  async function execute(name, args = {}, context = null) {
    return diaryContextStorage.run(context || {}, async () => dispatchTool(name, args, context));
  }

  async function dispatchTool(name, args, context = null) {
    const noteContext = noteContextFrom(context);
    switch (name) {
      case 'note.read': {
        if (!noteContext) return toolResult({ ok: false, summary: 'note tools require a bound document', errorCode: 'no_document' });
        return noteReadTool(args, noteContext);
      }
      case 'note.propose_edit': {
        if (!noteContext) return toolResult({ ok: false, summary: 'note tools require a bound document', errorCode: 'no_document' });
        return noteProposeEditTool(args, noteContext);
      }
      case 'knowledge.read':
      ...
      case 'knowledge.backlinks':
        return knowledgeBacklinksTool(args);
      ...
      case 'task.create': {
        ...
        if (recurrence && recurrence !== 'none' && !args.due_date) {
          return toolResult({ ok: false, summary: 'Recurring tasks require due_date', errorCode: 'invalid' });
        }
      ...
      case 'note_browser.tabs':
      case 'note_browser.read':
      case 'note_browser.screenshot':
      case 'note_browser.open':
      case 'note_browser.navigate':
      case 'note_browser.click':
      case 'note_browser.type':
      case 'note_browser.select':
      case 'note_browser.scroll':
      case 'note_browser.close':
        if (!noteBrowser?.available?.() || !context?.note?.documentId) return toolResult({ ok: false, summary: 'Note browser unavailable', errorCode: 'unavailable' });
        return noteBrowser.request(name, args, context.note.documentId);
      case 'memory.propose':
        if (!memory) return toolResult({ ok: false, summary: 'Memory service unavailable', errorCode: 'unavailable' });
        {
