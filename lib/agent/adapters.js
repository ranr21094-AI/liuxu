...
function createToolAdapters({ db, knowledgeSearch, hasDiaryAccessFlag, webSearch, webFetch, westockRun, imageGenerate, computer, chrome, memory }) {
  ...
  async function memorySearchTool(args = {}) {
    ...
    return toolResult({
      ok: true,
      summary: items.length ? `Found ${items.length} memory item(s)` : 'No matching memories',
