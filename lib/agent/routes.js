...
function registerAgentRoutes(app, ctx) {
  function diaryDocumentLocked(req, documentId) {
    ...
    return Boolean(doc?.visibility === 'diary' && !diaryUnlocked);
  }
  ...
  app.post('/api/agent/note-assist/:documentId/messages', async (req, res) => {
    try {
      ...
      const pack = runtimeFor(ctx.db, {
        hasDiaryAccessFlag: diaryUnlocked,
        modelClient: ctx.noteAssistModelClientFor
          ? await ctx.noteAssistModelClientFor(req)
          : (ctx.modelClientFor ? await ctx.modelClientFor(req) : undefined),
      });
  ...
  app.get('/api/agent/runs/:id/events', (req, res) => {
    try {
      ...
      if (diaryDocumentLocked(req, documentId)) {
