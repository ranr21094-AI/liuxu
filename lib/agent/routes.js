...
function registerAgentRoutes(app, ctx) {
  ...
  app.post('/api/agent/note-assist/:documentId/messages', async (req, res) => {
    try {
      ...
      let session = req.body?.newSession === true
        ? null
        : pack.store.findLatestSessionForDocument(documentId);
      if (!session) session = pack.store.createSession(raw.title || '留序 LiuXu', { documentId });
      const run = await pack.runtime.startNoteAssist({ session, documentId, userMessage: content });
