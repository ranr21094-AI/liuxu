...
function normalizeAgentAttachments(raw, isSafeUploadFilename) {
...
function handleAgentUploadError(err, res) {
  if (err instanceof require('multer').MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: '文件大小不能超过 30MB' });
    if (err.code === 'LIMIT_FILE_COUNT') return res.status(400).json({ error: '一次最多上传 14 张图片' });
