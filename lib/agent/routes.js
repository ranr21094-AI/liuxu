...
function runtimeFor(db, extras = {}) {
  ...
  if (extras.computer) entry.computer = extras.computer;
...
function registerAgentRoutes(app, ctx) {
  ...
  app.get('/api/agent/sessions', (req, res) => {
    try {
      ...
      res.json({
        sessions: store.listSessionSummaries({ includeArchived: req.query.archived === '1' }),
      });
    } catch (err) {
  ...
  app.get('/api/agent/sessions/:id', (req, res) => {
    try {
      ...
      const latestRun = store.getLatestRunForSession(session.id);
      res.json({
        ...session,
  ...
  app.get('/api/agent/tools', (req, res) => {
    ...
    res.json({ tools: definitions({ 'code.run': Boolean(computer), 'file.list': Boolean(computer) }) });
  });

  app.get('/api/agent/memories', (req, res) => {
    try {
      const { memory } = runtimeFor(ctx.db, { hasDiaryAccessFlag: ctx.hasDiaryAccess(req) });
      res.json({ items: memory.list({ layer: req.query.layer }) });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
