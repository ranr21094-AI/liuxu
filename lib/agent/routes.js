...
function persistNoteBrowserScreenshot(db, pending, run, input) {
  if (pending?.call?.name !== 'note_browser.screenshot' || !input?.data?.image) return input;
  const encoded = String(input.data.image || '');
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(encoded) || encoded.length > 8 * 1024 * 1024) throw new Error('浏览器截图数据无效或过大');
  const buffer = Buffer.from(encoded, 'base64');
  if (buffer.length < 3 || buffer[0] !== 0xff || buffer[1] !== 0xd8 || buffer[2] !== 0xff) throw new Error('浏览器截图不是有效的 JPEG 图片');
  const filename = `note-browser-${Date.now()}-${crypto.randomBytes(6).toString('hex')}.jpg`;
  const directory = path.join(db.dataDir, 'uploads');
  fs.mkdirSync(directory, { recursive: true });
  const filePath = path.join(directory, filename);
  fs.writeFileSync(filePath, buffer, { mode: 0o600 });
  try {
    if (run?.noteDocumentVisibility === 'diary') db.markPrivateUpload(filename);
  } catch (error) {
  ...
  return {
    ...
    data: {
      ...input.data,
      image: undefined,
      pageUrl: input.data.url,
      url: undefined,
      imageUrl,
      images: [{ url: imageUrl, mimeType: 'image/jpeg' }],
    },
  };
}

function isAgentFacingSession(session) {
  return Boolean(session) && !session.documentId;
}
...
function invalidateAgentRuntime(dataDir) {
  if (dataDir) runtimes.delete(dataDir);
}
...
function runtimeFor(db, extras = {}) {
  ...
  if (extras.noteBrowser) entry.noteBrowser = extras.noteBrowser;
  if (!entry.runtime) {
    entry.runtime = createRuntime({
      ...
      noteBrowser: {
        available() { return Boolean(entry.noteBrowser?.available?.()); },
        request(name, args, documentId) {
          return entry.noteBrowser?.request(name, args, documentId)
            || { ok: false, summary: 'Note browser unavailable', errorCode: 'unavailable' };
        },
      },
    });
  }
  return { ...entry, modelClient: extras.modelClient };
}

function registerAgentRoutes(app, ctx) {
  function diaryDocumentLocked(req, documentId, run = null) {
    if (!documentId) return false;
    const diaryUnlocked = ctx.hasDiaryAccess(req);
    const doc = serviceFor(ctx.db).knowledge.getDocument(documentId, { diaryUnlocked: true });
    return doc ? Boolean(doc.visibility === 'diary' && !diaryUnlocked) : !(run?.noteDocumentVisibility === 'standard' || (run?.noteDocumentVisibility === 'diary' && diaryUnlocked));
  }

  function runDocumentId(pack, run) {
    if (!run) return '';
    if (run.noteDocumentId) return String(run.noteDocumentId);
    if (!run.sessionId) return '';
    return pack.store.getSession(run.sessionId)?.documentId || '';
  }
  ...
  async function extras(req) {
    return {
      ...
      noteBrowser: ctx.noteBrowserFor?.(req),
    };
  }

  // Note-assist: per-document AI sidebar sessions (kind: note_assist).
  app.get('/api/agent/note-assist/:documentId/sessions', (req, res) => {
    try {
      const documentId = String(req.params.documentId || '');
      const diaryUnlocked = ctx.hasDiaryAccess(req);
      const doc = serviceFor(ctx.db).knowledge.getDocument(documentId, { diaryUnlocked: true });
      if (!doc) return res.status(404).json({ error: 'Document not found' });
      if (doc.visibility === 'diary' && !diaryUnlocked) return res.status(403).json({ error: 'Diary is locked' });
      const pack = runtimeFor(ctx.db, { hasDiaryAccessFlag: diaryUnlocked });
      res.json({ sessions: pack.store.sessionSummariesByDocument(documentId) });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });

  app.delete('/api/agent/note-assist/:documentId/sessions/:sessionId', (req, res) => {
    try {
      const documentId = String(req.params.documentId || '');
      ...
      if (doc.visibility === 'diary' && !diaryUnlocked) return res.status(403).json({ error: 'Diary is locked' });
      const pack = runtimeFor(ctx.db, { hasDiaryAccessFlag: diaryUnlocked });
      const session = pack.store.getSession(sessionId);
      if (!session || session.documentId !== documentId) return res.status(404).json({ error: 'Session not found' });
      const active = pack.store.listRunsForSession(sessionId)
        .find(run => ['queued', 'running', 'waiting_approval', 'waiting_client_tool', 'waiting_user'].includes(run.status));
      if (active) return res.status(409).json({ error: 'Session has an active run — stop it first' });
      const result = pack.store.deleteSessionHard(sessionId);
  ...
  app.get('/api/agent/note-assist/:documentId/session', (req, res) => {
    try {
      const documentId = String(req.params.documentId || '');
      const diaryUnlocked = ctx.hasDiaryAccess(req);
      const doc = serviceFor(ctx.db).knowledge.getDocument(documentId, { diaryUnlocked: true });
      ...
      if (doc.visibility === 'diary' && !diaryUnlocked) return res.status(403).json({ error: 'Diary is locked' });
      const pack = runtimeFor(ctx.db, { hasDiaryAccessFlag: diaryUnlocked });
      const requestedId = String(req.query.sessionId || '');
      let session = null;
      if (requestedId) {
        session = pack.store.getSession(requestedId) || null;
        if (session && session.documentId !== documentId) session = null;
      } else {
        session = pack.store.findLatestSessionForDocument(documentId);
      }
      if (!session) return res.status(404).json({ error: 'No assistant session for this document' });
      const activeRun = pack.store.listRunsForSession(session.id)
        .filter(run => ['queued', 'running', 'waiting_approval', 'waiting_client_tool', 'waiting_user'].includes(run.status))
        .at(-1) || null;
      res.json({
        session: {
          id: session.id,
  ...
  app.post('/api/agent/note-assist/:documentId/messages', async (req, res) => {
    try {
      const documentId = String(req.params.documentId || '');
      const content = String(req.body?.content || '').trim();
      if (!content) return res.status(400).json({ error: 'Message is required' });
      if (content.length > 8000) return res.status(400).json({ error: 'Message is too long (max 8000 characters)' });
      const diaryUnlocked = ctx.hasDiaryAccess(req);
      const { knowledge } = serviceFor(ctx.db);
      const raw = knowledge.getDocument(documentId, { diaryUnlocked: true });
      if (!raw) return res.status(404).json({ error: 'Document not found' });
      if (raw.visibility === 'diary' && !diaryUnlocked) return res.status(403).json({ error: 'Diary is locked' });
      const dependencies = await extras(req);
      if (ctx.noteAssistModelClientFor) dependencies.modelClient = await ctx.noteAssistModelClientFor(req);
      const pack = runtimeFor(ctx.db, { ...dependencies, hasDiaryAccessFlag: diaryUnlocked });
      ...
      if (!session) session = pack.store.createSession(raw.title || '留序 LiuXu', { documentId });
      const waiting = pack.store.listRunsForSession(session.id).slice().reverse().find(run => run.status === 'waiting_user');
      if (waiting) {
        const result = await pack.runtime.resumeUserInput(waiting.id, content);
        if (result.error) return res.status(409).json({ error: result.error });
        return res.status(202).json({ runId: waiting.id, sessionId: session.id, status: result.run.status, resumed: true });
      }
      const run = await pack.runtime.startNoteAssist({
        session,
        documentId,
        userMessage: content,
        modelClient: pack.modelClient,
        remoteClient: req.isRemoteClient === true,
      });
      if (run?.error) return res.status(run.status || 409).json({ error: run.error });
      res.status(202).json({ runId: run.id, sessionId: session.id, status: run.status });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.get('/api/agent/sessions/:id', (req, res) => {
    try {
      ...
      if (!isAgentFacingSession(session)) return res.status(404).json({ error: 'Session not found' });
  ...
  app.patch('/api/agent/sessions/:id', (req, res) => {
    try {
      const { store } = runtimeFor(ctx.db, { hasDiaryAccessFlag: ctx.hasDiaryAccess(req) });
      if (!isAgentFacingSession(store.getSession(req.params.id))) {
        return res.status(404).json({ error: 'Session not found' });
      }
  ...
  app.post('/api/agent/sessions/:id/messages', async (req, res) => {
    try {
      ...
      if (!isAgentFacingSession(session) || session.status === 'archived') return res.status(404).json({ error: 'Session not found' });
  ...
  app.get('/api/agent/runs/:id/events', (req, res) => {
    try {
      ...
      const run = pack.store.getRun(req.params.id);
      const documentId = runDocumentId(pack, run);
      if (diaryDocumentLocked(req, documentId, run)) {
      ...
      const unsubscribe = pack.runtime.attach(req.params.id, (event) => {
        if (diaryDocumentLocked(req, documentId, pack.store.getRun(req.params.id))) return;
  ...
  app.post('/api/agent/runs/:id/approvals/:approvalId', async (req, res) => {
    try {
      ...
      const pack = runtimeFor(ctx.db, await extras(req));
      if (diaryDocumentLocked(req, runDocumentId(pack, pack.store.getRun(req.params.id)))) return res.status(403).json({ error: 'Document is missing or diary is locked' });
  ...
  app.post('/api/agent/runs/:id/client-tools/:requestId/result', async (req, res) => {
    try {
      ...
      if (diaryDocumentLocked(req, runDocumentId(pack, pack.store.getRun(req.params.id)))) return res.status(403).json({ error: 'Document is missing or diary is locked' });
      const run = pack.store.getRun(req.params.id);
      const pending = run?.pendingClientTool;
      if (pending && pending.id !== req.params.requestId) return res.status(404).json({ error: 'Client tool request not found' });
      ...
      const rawResult = req.body?.result && typeof req.body.result === 'object' ? req.body.result : req.body || {};
      const clientResult = persistNoteBrowserScreenshot(ctx.db, pending, run, rawResult);
  ...
  app.get('/api/agent/tools', (req, res) => {
    ...
    const available = computerToolAvailability(Boolean(computer));
    for (const tool of definitions()) {
      if (tool.name.startsWith('note_browser.')) available[tool.name] = false;
      if (req.isRemoteClient && (tool.name.startsWith('browser.') || tool.name.startsWith('note_browser.'))) available[tool.name] = false;
    }
    res.json({ tools: definitions(available) });
  });
  ...
  app.put('/api/agent/memories/:id', (req, res) => {
    try {
      const { memory } = runtimeFor(ctx.db, { hasDiaryAccessFlag: ctx.hasDiaryAccess(req) });
      const result = memory.updateSystemContent(req.params.id, req.body?.content);
  ...
  app.post('/api/agent/memories/:id/restore', (req, res) => {
    try {
      const { memory } = runtimeFor(ctx.db, { hasDiaryAccessFlag: ctx.hasDiaryAccess(req) });
      const result = memory.restoreSystemContent(req.params.id);
      if (result.error) return res.status(result.status || 400).json({ error: result.error });
      res.json({ memory: result.memory });
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  app.delete('/api/agent/memories/:id', (req, res) => {
    try {
      ...
      if (result.error) {
        const status = result.error === 'System prompts cannot be deleted' ? 400 : 404;
  ...
  app.post('/api/agent/memory/refresh', async (req, res) => {
    try {
      ...
      const pack = runtimeFor(ctx.db, await extras(req));
      const result = await pack.runtime.startMemoryRefresh({ modelClient: pack.modelClient });
...
module.exports = { registerAgentRoutes, runtimeFor, invalidateAgentRuntime, persistNoteBrowserScreenshot };
