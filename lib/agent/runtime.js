...
const ABS_MAX_ROUNDS = 48;
const MEMORY_REFRESH_MAX_ROUNDS = 4;
const MEMORY_REFRESH_MAX_PROPOSALS = 5;
...
function clampMaxRounds(value, fallback = MAX_ROUNDS) {
  ...
  return Math.min(ABS_MAX_ROUNDS, Math.max(MIN_MAX_ROUNDS, Math.round(n)));
}
...
function createRuntime({ db, store, memory, modelClient, knowledgeSearch, hasDiaryAccessFlag, webSearch, westockRun, imageGenerate, computer, chrome }) {
  ...
  function runLimits(run) {
    ...
    if (run?.kind === 'memory_refresh') maxRounds = Math.min(MEMORY_REFRESH_MAX_ROUNDS, maxRounds);
  ...
  async function loop(run) {
    ...
    for (; run.round < limits.maxRounds;) {
      ...
      if (run.kind === 'memory_refresh') {
        const remaining = Math.max(0, MEMORY_REFRESH_MAX_PROPOSALS - Number(run.memoryProposals || 0));
  ...
  async function startMemoryRefresh() {
    ...
    const run = {
      ...
      status: 'queued',
      messages: [{ role: 'user', content: buildMemoryRefreshUserMessage(store, memory) }],
      events: [],
