...
const { requiresConfirmation, isAuto, definitions } = require('./tools');
...
function createRuntime({ db, store, memory, modelClient, knowledgeSearch, hasDiaryAccessFlag, webSearch, westockRun, computer, chrome }) {
  const emitters = new Map();
  const liveRuns = new Map();
  const adapters = createToolAdapters({
    db, knowledgeSearch, hasDiaryAccessFlag, webSearch, westockRun, computer, chrome, memory,
  });
  ...
  async function executeOne(run, call) {
    ...
    const result = await adapters.execute(call.name, call.arguments || {});
    if (call.name === 'memory.propose' && result?.ok) emit(run, 'memory.proposed', result.data);
  ...
  async function loop(run) {
    ...
    for (; run.round < MAX_ROUNDS;) {
      ...
      const calls = proposedCalls.filter(call => call && typeof call === 'object' && typeof call.name === 'string' && call.name.length <= 80);
      if (!calls.length) {
        run.status = 'completed';
        const envelope = parseActionEnvelope(modelResult.text);
        run.finalText = envelope?.answer || modelResult.text || '';
        ...
        const actionMessages = run.messages.filter(item => item.role === 'tool' && /^(task\.|file\.|code\.|browser\.)/.test(item.name || ''));
        if (actionMessages.length) {
          const evidence = actionMessages.flatMap(item => {
            try { return JSON.parse(item.content || '{}').evidence || []; } catch { return []; }
          }).slice(0, 16);
          if (evidence.length) {
            const proposal = memory.propose({
              runId: run.id,
              layer: 'L3',
              title: `任务流程：${run.goal.slice(0, 80)}`,
              content: run.finalText.slice(0, 2000),
              evidence,
            });
            if (!proposal.error) emit(run, 'memory.proposed', proposal.proposal);
          }
        }
      ...
      if (run.toolCalls + calls.length > MAX_TOOL_CALLS) {
        run.status = 'failed';
      ...
      const executed = await executeTools(run, calls.slice(0, MAX_TOOL_CALLS - run.toolCalls));
  ...
  return { start, attach, resolveApproval, cancel, clientToolResult, EVENT_TYPES };
}

module.exports = { createRuntime, parseActionEnvelope, MAX_ROUNDS, MAX_TOOL_CALLS };
