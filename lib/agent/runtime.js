...
function createRuntime({ db, store, memory, modelClient, knowledgeSearch, hasDiaryAccessFlag, webSearch, webFetch, westockRun, imageGenerate, computer, chrome, noteBrowser }) {
  ...
  async function executePreflight(run, call) {
    ...
    let denied = '';
    if (!toolsForRun(run).some(tool => tool.name === call.name)) denied = call.name === 'agent.delegate' && run.parentRunId ? 'Nested delegate is not allowed' : 'Tool is not available for this run';
  ...
  async function startNoteAssist({ session, documentId, userMessage, modelClient: requestClient } = {}) {
  ...
  async function start({ session, goal, userMessage, attachments = [], modelClient: requestClient } = {}) {
    const existing = findActiveRunForSession(session.id);
    if (existing) {
      return { error: 'Session already has an active run', status: 409 };
    }
