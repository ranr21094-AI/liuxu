...
const EVENT_TYPES = [
  ...
  'user_input.required', 'memory.proposed', 'delegate.started', 'delegate.completed',
...
function createRuntime({ db, store, memory, modelClient, knowledgeSearch, hasDiaryAccessFlag, webSearch, webFetch, westockRun, imageGenerate, computer, chrome }) {
  ...
  function findRun(runId) {
    ...
    if (run) liveRuns.set(run.id, run);
    return run || null;
  }
  ...
  function combinedToolCalls(run) {
    if (!run?.sharedBudgetRunId) return Number(run?.toolCalls) || 0;
    const parent = findRun(run.sharedBudgetRunId);
    return (Number(parent?.toolCalls) || 0) + (Number(run?.toolCalls) || 0);
  }
  ...
  async function runDelegate(parentRun, args = {}) {
    ...
    const limits = runLimits(parentRun);
    const parentRemainingRounds = Math.max(1, limits.maxRounds - (parentRun.round || 0));
    const requested = Number(args.maxRounds);
    const maxRoundsOverride = Math.min(
      MAX_DELEGATE_ROUNDS,
      parentRemainingRounds,
      Number.isFinite(requested) && requested > 0 ? Math.round(requested) : MAX_DELEGATE_ROUNDS,
    );
    ...
    const childRun = {
      ...
      round: 0,
      toolCalls: 0,
      sharedBudgetRunId: parentRun.id,
      maxRoundsOverride,
      createdAt: Date.now(),
    };
  ...
  async function driveRun(run) {
    ...
    for (; run.round < limits.maxRounds;) {
      ...
      const combined = combinedToolCalls(run);
      const remainingBudget = limits.maxToolCalls - combined;
      ...
      const repeatQuestion = shouldPauseForRepeatedMutations(run);
      if (repeatQuestion) {
  ...
  async function resolveApproval(runId, approvalId, { approved }) {
    ...
    if (approval.delegatedRunId) {
      ...
      store.saveRun(run);
      store.saveRun(child);
      if ((child.pendingApprovals || []).length) {
        bubbleWaitingState(child, run);
        return { run };
      }
  ...
  async function clientToolResult(runId, requestId, result) {
    ...
    if (run.pendingClientTool?.delegatedRunId) {
      ...
      if (!child || child.pendingClientTool?.id !== requestId) return { error: 'Client tool request not found' };
      emit(child, 'tool.completed', { requestId, result });
      child.messages.push({ role: 'tool', name: 'browser', content: JSON.stringify(result) });
      child.pendingClientTool = null;
      child.status = 'running';
      run.pendingClientTool = null;
      store.saveRun(child);
      store.saveRun(run);
  ...
  async function resumeUserInput(runId, userMessage) {
    const run = findRun(runId);
    if (!run) return { error: 'Run not found' };
    if (run.delegatedRunId && run.status === 'waiting_user') {
      ...
      run.status = 'running';
      store.saveRun(run);
      const childResult = await resumeUserInput(child.id, userMessage);
  ...
  return { start, startMemoryRefresh, attach, resolveApproval, cancel, clientToolResult, resumeUserInput, EVENT_TYPES, driveRun };
}
module.exports = { createRuntime, parseActionEnvelope, MAX_ROUNDS, MAX_TOOL_CALLS, clampMaxRounds, mergeVerified };
