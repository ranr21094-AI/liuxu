...
function messageText(content) {
  ...
  if (Array.isArray(content)) {
    return content.map(part => {
      ...
      if (part && typeof part === 'object' && ['text', 'input_text', 'output_text'].includes(part.type)) return String(part.text || '');
      return '';
    }).filter(Boolean).join('\n');
  }
...
function convertOpenAiUserContentToAnthropic(content) {
  if (typeof content === 'string') return content;
  ...
  return content.map(part => {
    ...
    if (part.type === 'document' && part.source) return part;
    if (part.type === 'image_url') {
      const image = dataUriParts(part.image_url?.url || part.image_url);
      return image ? { type: 'image', source: image } : null;
    }
    return null;
  }).filter(Boolean);
}
...
function convertOpenAiMessagesToAnthropic(messages = []) {
  ...
  for (const message of conversation) {
    ...
    output.push({
      ...
      content: message.role === 'user' ? convertOpenAiUserContentToAnthropic(message.content) : message.content,
    });
  }
...
function convertOpenAiUserContentToResponses(content) {
  if (typeof content === 'string') return [{ type: 'input_text', text: content }];
  if (!Array.isArray(content)) return [{ type: 'input_text', text: messageText(content) }];
  return content.map(part => {
...
function convertOpenAiMessagesToResponsesInput(messages = []) {
  ...
  for (const message of conversation) {
    ...
    const content = convertOpenAiUserContentToResponses(message.content);
