...
function createAgentStore(db) {
  ...
  function listSessions({ includeArchived = false, excludeDocumentBound = false } = {}) {
    const clauses = [];
    if (!includeArchived) clauses.push("status != 'archived'");
    if (excludeDocumentBound) clauses.push("document_id = ''");
    const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';
    return sqlite.prepare(`
      SELECT * FROM agent_sessions ${where} ORDER BY updated_at DESC
    `).all().map(row => hydrateSession(row));
  }
  ...
  function listSessionsByDocument(documentId) {
    ...
    `).all(id).map(hydrateSession);
  }

  function findLatestSessionForDocument(documentId) {
    return listSessionsByDocument(documentId)[0] || null;
  }

  function sessionSummariesByDocument(documentId) {
    const sessions = listSessionsByDocument(documentId);
    const countStatement = sqlite.prepare('SELECT COUNT(*) AS count FROM agent_messages WHERE session_id = ?');
    const firstMessageStatement = sqlite.prepare(`
      SELECT body FROM agent_messages WHERE session_id = ? ORDER BY sort_index ASC LIMIT 1
    `);
    return sessions.map(session => {
      const messageCount = countStatement.get(session.id)?.count || 0;
      let preview = '';
      if (messageCount) {
        const first = parseJson(firstMessageStatement.get(session.id)?.body, null);
        const content = typeof first?.content === 'string' ? first.content.trim() : '';
        preview = content.replace(/\s+/g, ' ').slice(0, 60);
      }
      return {
        id: session.id,
        title: session.title,
  ...
  function deleteSessionHard(sessionId) {
    const id = String(sessionId || '');
    const current = getSession(id);
    if (!current) return { error: 'Session not found', status: 404 };
    const tx = sqlite.transaction(() => {
      sqlite.prepare('DELETE FROM agent_runs WHERE session_id = ?').run(id);
      sqlite.prepare('DELETE FROM agent_messages WHERE session_id = ?').run(id);
  ...
  function listSessionSummaries(options = {}) {
    ...
    const clauses = ["(s.document_id IS NULL OR s.document_id = '')"];
    if (!includeArchived) clauses.push("s.status != 'archived'");
    const sessionWhere = `WHERE ${clauses.join(' AND ')}`;
  ...
  function createSession(title = '新任务', { documentId = '' } = {}) {
    const session = {
    ...
    sqlite.prepare(`
      INSERT INTO agent_sessions (id, title, status, checkpoint, created_at, updated_at, document_id)
      VALUES (?, ?, ?, NULL, ?, ?, ?)
    `).run(session.id, session.title, 'active', session.createdAt, session.updatedAt, session.documentId);
    if (!session.documentId) writeMeta(sqlite, 'active_session_id', session.id);
    return session;
  }
  ...
  function saveSession(session, { preserveTimestamps = false } = {}) {
    ...
    if (!exists) {
      ...
      `).run(
        ...
        next.updatedAt,
        String(next.documentId || ''),
      );
    } else {
      sqlite.prepare(`
        UPDATE agent_sessions
        SET title = ?, status = ?, checkpoint = ?, updated_at = ?, document_id = ?
        WHERE id = ?
      `).run(
        next.title || '新任务',
        next.status === 'archived' ? 'archived' : 'active',
        checkpointPayload,
        next.updatedAt,
        String(next.documentId || next.document_id || ''),
        next.id,
      );
    }
  ...
  function hydrateRun(row) {
    ...
    return {
      ...parsed,
      noteDocumentId: parsed.noteDocumentId || '',
      id: row.id,
