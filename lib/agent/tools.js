...
function definitions(available = {}) {
  const all = [
    ...
    {
      name: 'image.generate',
      description: 'Generate images after user approval. Backend follows saved image provider (Seedream or Getoken). Getoken models: gpt-image-2, grok-imagine-image, nano-banana-2 (each needs its own API key in settings). Getoken: prompt + optional image for edits; use n for multiple images in one call (max 4). Seedream-only: batch/batch_size, sequential_image_generation, layer_decomposition, web_search, transparent background. Pass user attachment URLs in image for image-to-image.',
      auto: false,
      parameters: {
        type: 'object',
        additionalProperties: false,
        ...
        properties: {
          ...
          model: {
            type: 'string',
            description: 'Model id. Getoken: gpt-image-2 | grok-imagine-image | nano-banana-2 (each key in settings). Seedream: doubao-seedream-* ids. Omit to use saved default.',
          },
          size: { type: 'string', description: 'Size keyword or WxH / auto' },
          quality: { type: 'string', enum: ['standard', 'high'], description: 'Getoken only: image quality' },
          n: { type: 'integer', description: 'Getoken only: number of images (1-4)' },
          watermark: { type: 'boolean', description: 'Seedream only: add AI watermark badge' },
          image: {
            description: 'Reference image URL, data URI, or array of up to 14 references',
