...
function getokenEndpoint(baseUrl, resource) {
  const root = String(baseUrl || '').replace(/\/+$/, '');
  return `${root.endsWith('/v1') ? root : `${root}/v1`}/images/${resource}`;
}

async function readJsonWithLimit(response, maxBytes = 32 * 1024 * 1024) {
  const declared = Number(response.headers?.get?.('content-length'));
  if (Number.isFinite(declared) && declared > maxBytes) throw new Error('Image provider response is too large');
  if (!response.body?.getReader) {
    const text = await response.text();
    if (Buffer.byteLength(text) > maxBytes) throw new Error('Image provider response is too large');
  ...
  try { return JSON.parse(Buffer.concat(chunks, total).toString('utf8')); } catch { return {}; }
}
...
async function requestGetokenGeneration(options, { fetchImpl, baseUrl, timeoutMs = 300000 }) {
  ...
  const response = await fetchFn(getokenEndpoint(baseUrl, 'generations'), {
    method: 'POST',
    redirect: 'error',
    headers: {
...
async function requestGetokenEdit(options, { fetchImpl, baseUrl, dataDir, isSafeUploadFilename, timeoutMs = 300000 }) {
  ...
  const response = await fetchFn(getokenEndpoint(baseUrl, 'edits'), {
    method: 'POST',
    redirect: 'error',
    headers: {
      Authorization: `Bearer ${options.apiKey}`,
