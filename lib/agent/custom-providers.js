...
const { isPrivateIpLiteral, isLikelyProxySyntheticResolution } = require('../net/ssrf');
...
async function validateProviderBaseUrl(url, lookupFn) {
  ...
  const lookup = lookupFn || dnsLookup;
  let addresses;
  try {
  ...
  if (!list.length) return { error: 'Base URL hostname could not be resolved' };
  if (list.some(entry => isPrivateIpLiteral(entry.address || entry)) && !isLikelyProxySyntheticResolution(list)) {
    return { error: 'Base URL hostname resolved to a private or local address' };
  }
  return { value: normalized };
}
