...
const CUSTOM_MODEL_ID_PATTERN = /^custom\/([a-z0-9_-]{1,32})\/([a-zA-Z0-9._:+-]{1,120})$/;
...
async function validateProviderBaseUrl(url) {
  ...
  if (parsed.protocol === 'http:') {
    if (!isLoopbackHostname(hostname)) {
      return { error: 'HTTP base URL is only allowed for localhost' };
    }
    return { value: normalized };
  }
  if (isPrivateIpLiteral(hostname)) {
    return { error: 'HTTPS base URL must not target a private network address' };
  }
  try {
    const resolved = await dnsLookup(hostname, { all: true });
    if (resolved.some(entry => isPrivateIpLiteral(entry.address))) {
      return { error: 'HTTPS base URL must not resolve to a private network address' };
    }
  } catch {
...
function normalizeCustomProvider(raw = {}, existing = null) {
  ...
  if (!name || !baseUrl || !models.length) return null;
  return {
...
function sanitizeCustomProvidersSync(input, current = []) {
  ...
  const next = [];
  for (const raw of source.slice(0, MAX_CUSTOM_PROVIDERS)) {
