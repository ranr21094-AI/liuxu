...
// Some system-wide proxy/TUN clients answer public DNS queries with addresses
// from RFC 2544's 198.18.0.0/15 benchmarking range (and sometimes an IPv6 ULA).
// They are safe to accept only as a complete synthetic DNS answer for an HTTPS
// hostname, where TLS still authenticates the original hostname.
function isProxySyntheticIpLiteral(address) {
  const value = String(address?.address || address || '').trim();
...
function isUniqueLocalIpv6Literal(address) {
  let value = String(address?.address || address || '').trim().toLowerCase();
  if (value.startsWith('[') && value.endsWith(']')) value = value.slice(1, -1);
  return net.isIP(value) === 6 && (value.startsWith('fc') || value.startsWith('fd'));
}

function isLikelyProxySyntheticResolution(addresses) {
  const list = Array.isArray(addresses) ? addresses : (addresses ? [addresses] : []);
  return list.some(isProxySyntheticIpLiteral)
    && list.every(entry => isProxySyntheticIpLiteral(entry) || isUniqueLocalIpv6Literal(entry));
}
...
function unresolvedHostnameError() {
  const error = new Error('Hostname could not be resolved');
  error.code = 'SSRF_UNRESOLVED';
  return error;
}

async function hostnameResolvesToPrivateOrLocal(hostname, lookupFn, { allowProxySynthetic = false } = {}) {
  ...
  try {
    ...
    throw unresolvedHostnameError();
  }
  if (!Array.isArray(addresses) || !addresses.length) {
    throw unresolvedHostnameError();
  }
  if (allowProxySynthetic && isLikelyProxySyntheticResolution(addresses)) return false;
...
async function validateGeneratedImageUrl(value, lookupFn) {
  ...
  try {
    if (await hostnameResolvesToPrivateOrLocal(url.hostname, lookupFn, { allowProxySynthetic: true })) {
      throw new Error('Generated image URL is not allowed');
    }
  } catch (error) {
    throw new Error('Generated image URL is not allowed');
...
// Public hostnames must not resolve to private/loopback addresses (DNS rebinding).
// Literal localhost / private IPs are allowed so local models and LAN tools still work.
async function assertPublicHostname(hostname, lookupFn) {
  if (hostnameIsBlockedLiteral(hostname)) {
    throw new Error('URL hostname resolved to a private or local address');
  }
  if (await hostnameResolvesToPrivateOrLocal(hostname, lookupFn)) {
    throw new Error('URL hostname resolved to a private or local address');
  }
}

function createAgentWebFetchValidator(originValue) {
  const origin = parseHttpUrl(originValue);
  const originPrivate = origin ? hostnameIsBlockedLiteral(origin.hostname) : false;
  const originHost = String(origin?.hostname || '').toLowerCase();
  return async function validateAgentWebFetchUrl(value, lookupFn) {
    ...
    const hopPrivate = hostnameIsBlockedLiteral(url.hostname);
    if (originPrivate) {
      if (url.hostname.toLowerCase() !== originHost) {
        throw new Error('Local web.fetch redirects must stay on the same host');
      }
      return url.toString();
    }
    if (hopPrivate) throw new Error('URL is not allowed');
    await assertPublicHostname(url.hostname, lookupFn);
    return url.toString();
  };
}
...
async function fetchFollowingRedirects(url, {
  ...
  if (typeof validate !== 'function') throw new Error('URL validator is required');
  let current = url;
  for (let hop = 0; hop < maxRedirects; hop++) {
    const safeUrl = await validate(current, lookupFn);
    const response = await fetchFn(safeUrl, { ...init, redirect: 'manual' });
...
module.exports = {
  isPrivateIpLiteral,
  isProxySyntheticIpLiteral,
  isLikelyProxySyntheticResolution,
  hostnameIsBlockedLiteral,
  hostnameResolvesToPrivateOrLocal,
  validateGeneratedImageUrl,
  assertPublicHostname,
  createAgentWebFetchValidator,
