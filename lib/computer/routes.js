const { loadPolicy, savePolicy, computerToolsAllowed } = require('./policy');
...
function registerComputerRoutes(app, { db }) {
  app.get('/api/admin/agent-policy', (_req, res) => {
  ...
  app.put('/api/admin/agent-policy', (req, res) => {
    if (req.isRemoteClient) return res.status(403).json({ error: '目录白名单只能在电脑端修改' });
    const dataDir = db.dataDir;
    try {
  ...
  app.post('/api/agent/chrome/pairing', (req, res) => {
    if (req.isRemoteClient) return res.status(403).json({ error: 'Chrome 配对只能在电脑端完成' });
    const dataDir = db.dataDir;
    const chrome = createChromeBridge(dataDir);
    res.json(chrome.startPairing());
  });

  app.post('/api/agent/chrome/pairing/confirm', (req, res) => {
    if (req.isRemoteClient) return res.status(403).json({ error: 'Chrome 配对只能在电脑端完成' });
    const dataDir = db.dataDir;
    const chrome = createChromeBridge(dataDir);
