...
function resolvePythonExecutable({
  ...
  const configured = typeof env.PYTHON_PATH === 'string' ? env.PYTHON_PATH.trim() : '';
  if (configured) return configured;
  const candidates = platform === 'win32' ? ['python'] : ['python3', 'python'];
  return candidates.find(canExecute) || candidates[0];
}

function resolveCodeCommand(language, options = {}) {
  const platform = options.platform || process.platform;
  if (language === 'python') {
    return { cmd: resolvePythonExecutable({ ...options, platform }), argvPrefix: ['-I'] };
  }
  return {
    cmd: platform === 'win32' ? 'powershell.exe' : 'pwsh',
    argvPrefix: platform === 'win32'
...
function killTree(child) {
  ...
  try {
    if (process.platform === 'win32') {
      ...
      // The child is spawned as a process group leader (detached); if the group
      // kill fails (e.g. ESRCH because the group is gone), fall back to the
      // direct signal so a still-running child does not survive the timeout.
      try {
...
function createCodeRunner({ accountId, workdir, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  return {
    async execute(_name, args = {}) {
      ...
      const defaultLanguage = process.platform === 'win32' ? 'powershell' : 'python';
      const language = args.language === 'python' || args.language === 'powershell'
        ? args.language
        : defaultLanguage;
      ...
      const resolvedCommand = resolveCodeCommand(language);
      const command = { cmd: resolvedCommand.cmd, argv: [...resolvedCommand.argvPrefix, file] };
        ...
        timer = setTimeout(() => {
          ...
          killGrace = setTimeout(() => settle(toolResult({
            ...
            data: {
              ...
              risk: 'Native script execution is not a sandbox.',
            },
            errorCode: 'timeout',
          })), KILL_GRACE_MS);
        }, timeout);
