...
function createCodeRunner({ accountId, workdir, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  return {
    async execute(_name, args = {}) {
      if (runningByAccount.get(accountId)) {
      ...
      if (!script.trim()) return toolResult({ ok: false, summary: 'Script is required', errorCode: 'invalid' });
      ...
      return await new Promise((resolve) => {
        runningByAccount.set(accountId, true);
...
module.exports = { createCodeRunner, DEFAULT_TIMEOUT_MS, HARD_TIMEOUT_MS };
