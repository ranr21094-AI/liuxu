...
function computerToolsAllowed(req, dataDir) {
  ...
  if (req.user?.role !== 'admin') return { ok: false, error: 'Admin role required' };
  if (!isLoopback(req)) return { ok: false, error: 'Loopback access required' };
  if (!recentlyReauthed(req.user?.id)) return { ok: false, error: 'Recent password re-authentication required' };
...
function resolveAllowed(target, allowedDirectories) {
  ...
  const allowed = allowedDirectories.map(dir => fs.realpathSync(path.resolve(dir)));
