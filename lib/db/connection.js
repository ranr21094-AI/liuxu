...
function migrationSqlFor(version) {
  const suffix = Number(version) === 2 ? 'performance' : Number(version) === 3 ? 'knowledge_links_history' : Number(version) === 4 ? 'note_assistant' : '';
...
// ALTER TABLE ADD COLUMN has no IF NOT EXISTS in SQLite, so the note-assistant
// column is ensured from JS; the v4 SQL file only carries idempotent statements.
function ensureNoteAssistantColumns(sqlite) {
  const columns = sqlite.prepare('PRAGMA table_info(agent_sessions)').all().map(column => column.name);
  if (!columns.includes('document_id')) {
    sqlite.exec("ALTER TABLE agent_sessions ADD COLUMN document_id TEXT NOT NULL DEFAULT ''");
  }
}
...
function applySchema(sqlite) {
  const migrate = sqlite.transaction(() => {
    ...
    if (current >= SCHEMA_VERSION && SCHEMA_VERSION >= 2) {
      if (SCHEMA_VERSION >= 4) ensureNoteAssistantColumns(sqlite);
      sqlite.exec(migrationSqlFor(SCHEMA_VERSION));
    }
    for (let version = current + 1; version <= SCHEMA_VERSION; version += 1) {
      if (version === 4) ensureNoteAssistantColumns(sqlite);
