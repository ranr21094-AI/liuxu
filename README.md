...
1. 在最新版本页面下载 `LiuXu-Setup-1.2.10-x64.exe`。
2. 双击安装包，按向导选择程序安装位置。
...
```powershell
Get-FileHash .\LiuXu-Setup-1.2.10-x64.exe -Algorithm SHA256
```
...
```text
LiuXu-1.2.10-mac-arm64.dmg
LiuXu-1.2.10-mac-arm64.zip
```
...
```bash
shasum -a 256 -c LiuXu-1.2.10-mac-arm64.dmg.sha256
shasum -a 256 -c LiuXu-1.2.10-mac-arm64.zip.sha256
```
...
`v1.2.10` 测试安装包可能未完成正式签名，这是当前版本的已知限制。请从本仓库 Release 页面下载并核对 SHA-256，不要使用来源不明的转载包。
...
```text
LiuXu-Setup-1.2.10-x64.exe
LiuXu-Setup-1.2.10-x64.exe.sha256
desktop-build-summary.json
LiuXu-1.2.10-mac-arm64.dmg
LiuXu-1.2.10-mac-arm64.dmg.sha256
LiuXu-1.2.10-mac-arm64.zip
LiuXu-1.2.10-mac-arm64.zip.sha256
