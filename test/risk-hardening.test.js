...
test('AI chat frontend supports local history and refreshed workspace layout', () => {
  ...
  assert.match(styleSource, /\.ai-chat-composer\s*\{[\s\S]*border-radius:\s*14px;/);
  ...
  assert.match(styleSource, /\.ai-chat-view::after\s*\{[\s\S]*background:\s*#fff;/);
  ...
  assert.match(styleSource, /\.ai-chat-composer\s*\{[\s\S]*border:\s*1px solid rgba\(148, 163, 184, 0\.16\);[\s\S]*background:\s*var\(--color-card\);/);
  assert.match(styleSource, /\.ai-chat-composer textarea\s*\{[\s\S]*min-height:\s*64px;[\s\S]*max-height:\s*112px;/);
  assert.match(styleSource, /\.ai-chat-composer-footer\s*\{[\s\S]*border-top:\s*1px solid rgba\(148, 163, 184, 0\.18\);/);
  assert.match(styleSource, /\.ai-chat-web-toggle\s*\{[\s\S]*min-height:\s*32px;[\s\S]*background:\s*rgba\(248, 250, 252, 0\.92\);/);
  ...
  assert.match(styleSource, /\.ai-chat-body\s*\{[\s\S]*overflow:\s*hidden;/);
  assert.match(styleSource, /\.ai-chat-body:has\(\.ai-chat-empty\)\s*\{[\s\S]*flex:\s*0 0 248px;/);
  assert.match(styleSource, /\.ai-chat-empty-copy\s*\{[\s\S]*display:\s*grid;[\s\S]*gap:\s*8px;/);
  assert.match(styleSource, /\.ai-message\.user\s*\{[\s\S]*margin-top:\s*22px;/);
  assert.match(styleSource, /\.ai-message\.user \.ai-message-bubble\s*\{[\s\S]*max-width:\s*min\(560px, 72%\);/);
  ...
  assert.match(styleSource, /\.ai-message\.assistant \.ai-message-bubble\s*\{[\s\S]*max-width:\s*min\(100%, 820px\);/);
  assert.match(styleSource, /\.ai-message\.user \.ai-message-content\s*\{[\s\S]*background:\s*rgba\(219, 234, 254, 0\.86\);[\s\S]*color:\s*#0f172a;/);
  ...
  assert.match(styleSource, /@media \(max-width: 768px\)[\s\S]*\.ai-chat-web-toggle\s*\{[\s\S]*left:\s*10px;[\s\S]*bottom:\s*calc\(10px \+ env\(safe-area-inset-bottom\)\);/);
